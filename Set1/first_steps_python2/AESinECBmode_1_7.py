...
def KeyExpansion(key_byte):
	...
	for i in range(0,4):
		...
		#print ''.join('{:02x}'.format(x) for x in w[i])
	for j in range(4,44):
		tmp = w[j-1]
		#print 'tmp:', ''.join('{:02x}'.format(x) for x in tmp)
		if j%4 == 0:
			tmp = bytearray(SubWord(RotWord(tmp))[k] ^ RCON(j/4)[k] for k in range(0,4))
			#print 'RotWord', ''.join('{:02x}'.format(x) for x in RotWord(tmp))
			#print 'SubWord', ''.join('{:02x}'.format(x) for x in SubWord(RotWord(tmp)))
			#print 'after rcon:', ''.join('{:02x}'.format(x) for x in tmp)
		w.append(bytearray(w[j-4][k] ^ tmp[k] for k in range(0,4))) #w[j] = w[j-4] ^ tmp
		#new_w = bytearray(w[j-4][k] ^ tmp[k] for k in range(0,4))
		#print 'new_w:', ''.join('{:02x}'.format(x) for x in new_w)
	keys = [] # 11 keys 4x4 bytes matrix
	for a in range(len(w)/4):
...
encrypted_data_hex = encrypted_data_base64.decode("base64").encode("hex")
