...
def InvRoundBlock(state, key_expanded):
...
f = open('sources/7_encrypt_mode.txt', 'r')
plain_text = ""
for line in f:
	plain_text += line
# blocks of 128 bits / 16 bytes
plain_text_hex = hexlify(plain_text.encode('utf-8'))
# blocks of 128 bits / 16 bytes
...
for state in states:
	...
	encrypted += d
encrypted_b64 = b2a_base64(unhexlify(encrypted))
print(len(encrypted_b64))
for i in range(int(len(encrypted_b64)/60)+1):
	print(encrypted_b64[60*i:60*(i+1)].decode('utf-8'))